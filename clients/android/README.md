...
The phone does not need Google login or `gcloud`.
...
the public command name. On Android the service forces the Google API route to a
fronted pinned mode so the carrier path does not depend on Android's local
resolver during startup.
